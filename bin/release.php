...
php {$GLOBALS['argv'][0]} --new=[major|point] --update=RELEASE_NUM 
  ...
  - Publish a GitHub tag/release to the wp.org svn
    ...
    --new [major|point]
        Is it a major or point release?
...
if ( empty( $args ) ) {
	...
	execute_command( "git branch -l release/*", "No tags found." );
}
else if ( isset( $args['new'] ) ) {
	$new = $args['new'];
	if ( empty( $new ) ) {
		$new = prompt( "What type of release?\n", [ 'major', 'point' ]  );
	}
	...
	if ( 'major' == $new ) {
		...
		// @todo
		fail( "Still needs doing :(\n" );
		// Ask which version to build a point off of

		// Check out to the version's release branch
...
} else if ( isset( $args['publish'] ) ) {
	$publish = $args['new'];

	// Get/check the version
	$version = prompt( "What version are you publishing?\n" );
	if ( empty( $publish ) ) {
		$publish = prompt( "Where would you like to publish this release?\n", [ 'git', 'svn' ]  );
	}

	if ( 'git' == $publish ) {
		// @todo Build the plugin in a standardized way
		fail( "@todo: Publish tag to SVN\n" );
//		build_or_update_production_release_branch( $version );
	} else if ( 'svn' == $publish ) {
		// @todo Build the plugin in a standardized way
		fail( "@todo: Publish tag to SVN\n" );
